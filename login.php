<?php
    ...
    if(count($_POST)>0) {
    	...
    	$result = mysqli_query($conn,"SELECT * FROM login_user WHERE Email='" . $_POST["email"] . "' and Password = '". $_POST["pass"]."'");
    	$count  = mysqli_num_rows($result);
    	if($count==0) {

        $result = mysqli_query($conn,"SELECT `name` FROM `staff_login`  WHERE email='" . $_POST["email"] . "' and Password = '". $_POST["pass"]."'");
        $count= mysqli_num_rows($result);
        if($count!=0){
